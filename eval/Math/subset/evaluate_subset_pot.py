...
def generate_sample_batch(question_list, llm, batch_size=512):
    ...
    completions = []
    for i in tqdm(range(0, len(question_list), batch_size), desc="Generating batches"):
        batch = question_list[i:i + batch_size]
        outputs = llm.generate(batch, sampling_params)
        batch_completions = [output.outputs[0].text.strip() for output in outputs]
        batch_completions = postprocess_completions(batch_completions)
...
def run(args, max=-1):
    ...
    for file in files:
        ...
        if file not in other_set:
            ...
            completions = generate_sample_batch(conv_list, llm, batch_size=512)
